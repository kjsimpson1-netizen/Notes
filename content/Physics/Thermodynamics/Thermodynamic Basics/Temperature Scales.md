>[!Warning]
>There is a difference between an actual temperature and a temperature interval. A temperature is how warm/cold the object would feel, and temperature interval is a change of temperature. 
#### Celsius
To make thermometers useful, there has to be a scale on them. The Celsius scale was found by marking the thermometer measuring water at it's freezing point and calling it $0\degree\text{C}$, then doing the same with waters boiling point and calling it $100\degree\text{C}$, then dividing the space between the two markings into 100 equal parts and calling each one a degree Celsius.
#### Kelvin
...
### Defining the Kelvin Scale
#### The triple point of water
When the [[Pressure]] above water decreases, so does the temperature at which the water will boil because the [[Pressure]] holding the water together decreases, and therefore takes less energy to overcome the dipole-dipole forces holding the water together as a liquid. If we take this [[Pressure]] to the extreme lows, we can reach a point when the water is at a point where it is boiling, freezing and liquid all at the same time. 
