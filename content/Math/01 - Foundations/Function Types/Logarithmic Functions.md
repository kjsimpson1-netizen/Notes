...
We can use [[Logarithm and Exponential Rules]] to simplify many of these expressions. 
Some other definitive characteristic is that $f(1)=0$ for all logarithms, and that $\log_n(n)=1$. The tailing end of a logarithm never actually has a slope of 0 and is constantly increasing.
>[!Note]
>From now on, $\log(x)=\ln(x)$
